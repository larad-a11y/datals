...
export function DashboardAIChat({ kpis, tunnels, charges, salaries, coachingExpenses, selectedMonth }: DashboardAIChatProps) {
  ...
  const buildContext = () => ({
    ...
    charges: {
      paymentFeesPercent: charges.paymentFeesPercent,
      closerCommissionPercent: charges.closerCommissionPercent,
      agencyCommissionPercent: charges.agencyCommissionPercent,
      associateCommissionPercent: charges.associateCommissionPercent,
      otherFixedCharges: charges.otherFixedCharges,
    },
    salariesCount: salaries.length,
    totalSalaries: salaries.reduce((s, x) => s + x.amount, 0),
